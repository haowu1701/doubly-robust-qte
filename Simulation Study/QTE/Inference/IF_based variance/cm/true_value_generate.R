...
set.seed(37203)
# Large sample size used to approximate true values.
# This script generates true_values.RData locally; the generated .RData file is not included in the repository.
BigN = 1e8
...
p0_final = valid_pvals$p0[1:BigN]; p1_final = valid_pvals$p1[1:BigN]
# Transform the valid p-values to chi-square distributed outcomes (df = 5)
Y0_samp = qchisq(p0_final, df = 5); Y1_samp = qchisq(p1_final, df = 5)
...
#true_values.RData is generated locally and not included in GitHub.
save(
   # Y1_samp, Y0_samp,
     m_Y0_0.25, m_Y1_0.25, QTE_0.25_true, 
